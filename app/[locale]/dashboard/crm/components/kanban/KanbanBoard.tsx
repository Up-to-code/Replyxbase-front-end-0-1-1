    ...
    <div className="flex gap-4 overflow-x-auto pb-4 px-4 min-h-[calc(100vh-200px)]">
      {defaultColumns.map((column) => {
        ...
        return (
          <div
            key={column.id}
            className="min-w-[320px] max-w-[320px] flex flex-col rounded-xl border-2 border-slate-200 bg-slate-50/30"
            ...
            <div className="p-4 border-b-2 border-slate-200 bg-white rounded-t-xl sticky top-0 z-10">
              <div className="flex items-center justify-between">
                {editingColumnId === column.id ? (
                  ...
                  <div 
                    className="font-semibold text-slate-900 flex items-center gap-2 cursor-pointer hover:text-[#005bbc] transition-colors"
            ...
            <div className="p-3 space-y-3 flex-1 overflow-y-auto custom-scrollbar">
              {columnBookings.map((booking) => (
                <motion.div
                  ...
                  } ${!editingBookingId ? 'cursor-grab active:cursor-grabbing hover:border-slate-300' : ''}`}
